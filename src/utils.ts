...
import {evmToAddress, decodeAddress} from "@polkadot/util-crypto";
...
const ASTAR_SS58FORMAT: number = 5;

export function evmToNativeAddress(evmAddr : string): string {
    return evmToAddress(evmAddr, ASTAR_SS58FORMAT, 'blake2');
}
export function decordPubKeyToNativeAddress(pubKey : string): string {
    return decodeAddress(pubKey, false, ASTAR_SS58FORMAT).toString();
}